...
| Webhook/callback hits 404 or old code | Callback URL built from `APP_URL`/production domain instead of the current deployment | Derive from the request's `x-forwarded-host` header like `api/routers/maya-webhook.ts:46` does (`06237e4`, `f8965c8`, `d99dfcf`) |
...
2. `@contracts/*` alias not resolving in the esbuild bundle — that's why the `build` script has the `createRequire` banner.
