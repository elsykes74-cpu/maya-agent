...
export function AgentTile({ title = 'Agent', description, icon = 'Zap', iconColor = C.teal, iconBg = C.tealS, tileBg = 'light', status, count, lastRun, isLoading, isDisabled, onClick, className = '', style }: AgentTileProps) {
  ...
  return (
    <button
      ...
      style={{ padding: 0, height: 158, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, position: 'relative', border: 'none', cursor: isDisabled ? 'not-allowed' : 'pointer', opacity: isDisabled ? 0.5 : 1, width: '100%', ...style }}
    >
      {status && (
      ...
      <MayaIconBadge bg={iconBg} size={62} round={16}>
        <IconComponent size={28} color={iconColor} strokeWidth={2} />
      </MayaIconBadge>
      <span style={{ fontSize: 13, fontWeight: 700, color: textColor, textAlign: 'center', padding: '0 8px', lineHeight: 1.3, letterSpacing: '-0.015em' }}>{title}</span>
